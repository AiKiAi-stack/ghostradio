...
| 状态 | 改进项 | 说明 |
|------|-------|------|
| ✅ | **原子化任务队列** | 将 `queue.txt` 升级为基于目录的任务文件，解决并发丢失风险。 |
| ✅ | **集中化元数据索引** | 引入 `metadata.json` 记录节目详情，避免频繁扫描文件系统。 |
| ✅ | **音频时长自动检测** | 集成 `mutagen` 自动提取生成音频的准确时长。 |
| ✅ | **RSS 自动更新** | Worker 完成后自动触发生命周期回调，更新 RSS Feed。 |
| ✅ | **细粒度重试机制** | 集成 `Tenacity` 实现网络请求的指数退避重试。 |
| ✅ | **实时进度上报** | Worker 主动更新任务状态文件，提供真实进度反馈。 |
| ✅ | **健康监测系统** | 提供 `/health` 系列端点监控系统资源和 Worker 状态。 |
| ✅ | **任务重试策略** | 失败任务自动进入重试队列，超限后归档至 `failed/`。 |
...
| 状态 | 改进项 | 说明 |
|------|-------|------|
| ✅ | **准确时长提取** | 使用 `mutagen` 自动提取生成音频的准确时长。 |
| ✅ | **多用户隔离** | 支持 `user_id` 字段，实现数据、日志和 RSS 的用户级隔离。 |
| ✅ | **存储配额管理** | 每个用户限制 10 条播客，到达上限后自动执行 FIFO 清理。 |
| ✅ | **订阅二维码** | 支持生成 RSS 订阅二维码，扫码即可接入 iPhone 播客 App。 |
| ✅ | **实时进度上报** | Worker 主动更新任务状态文件，提供真实进度反馈。 |
| ✅ | **通知回调 (Webhooks)** | 任务成功或失败时触发外部 Webhook（如 Telegram, Slack）。 |
...
### 10.1 部署加固 🚧
- [ ] **日志轮转**: 将 `FileHandler` 升级为 `RotatingFileHandler`。
- [ ] **元数据同步清理**: 音频文件被 FIFO 删除时，同步更新 `metadata.json`。
